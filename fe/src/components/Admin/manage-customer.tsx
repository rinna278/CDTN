...
const ManageCustomer = () => {
  ...
  // ✅ CHANGE 2: Tự động lưu vào LocalStorage mỗi khi danh sách xóa thay đổi
  useEffect(() => {
  ...
  const handleDeleteUser = (user: AllUser) => {
    setCustomers((prev) => prev.filter((item) => item.id !== user.id));
    setDeleteCustomers((prev) => [...prev, user]);
  };

  const handleRestoreUser = (user: AllUser) => {
    setDeleteCustomers((prev) => prev.filter((item) => item.id !== user.id));
    setCustomers((prev) => [...prev, user]);
  };

  const handleRestoreAll = () => {
    setCustomers((prev) => [...prev, ...deleteCustomers]);
    setDeleteCustomers([]);
    // LocalStorage sẽ tự clear nhờ useEffect ở trên
  };

  // ✅ CHANGE 3: Logic gọi API và lọc dữ liệu
  useEffect(() => {
    const fetchAllUser = async () => {
      try {
        ...
        // ⚠️ QUAN TRỌNG: Lọc những user đã xóa ra khỏi danh sách Active
        // Lấy danh sách ID đã xóa từ state hiện tại (được load từ localStorage)
        const deletedIds = new Set(deleteCustomers.map((u) => u.id));

        // Chỉ giữ lại những user KHÔNG có trong danh sách đã xóa
  ...
  }, []); // Chỉ chạy 1 lần khi mount, lúc này deleteCustomers đã được init từ localStorage
