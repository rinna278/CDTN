...
const ManageOrder = () => {
    ...
    const [stats, setStats] = useState({
        pending: 0, confirmed: 0, processing: 0, shipping: 0, delivered: 0, cancelled: 0, refunded: 0
    })
    ...
    const handleUpdateStatus = async() => {
        ...
        try {
            ...
            if (axios.isAxiosError(error)){
                alert(error?.response?.data?.message || "Không tìm thấy mã đơn hàng hoặc có lỗi xảy ra");
            }
            else{
        ...
        try{
            ...
            if (res && res.data) {
                ...
                const newStats = {
                    ...
                    confirmed: res.data.filter(o => o.orderStatus === 'confirmed').length,
                    processing: res.data.filter(o => o.orderStatus === 'processing').length,
                    shipping: res.data.filter(o => o.orderStatus === 'shipping').length,
                    delivered: res.data.filter(o => o.orderStatus === 'delivered').length,
                    cancelled: res.data.filter(o => o.orderStatus === 'cancelled').length,
