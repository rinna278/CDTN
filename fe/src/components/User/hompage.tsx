...
const HomePage = () => {
  ...
  // Lấy sản phẩm có discount cao nhất (tiệm cận 30%)
  const fetchDiscountProducts = async () => {
    try {
      const response = await getAllProduct({
        page: 1,
        limit: 100, // Lấy nhiều sản phẩm để lọc
        status: 1,
      });

      console.log("📦 Total products:", response.data?.length);

      // Lọc sản phẩm có discount từ 1% đến 30%
      const filtered =
      ...
      // ✅ Sắp xếp theo discount giảm dần ở Frontend
      ...
      setDiscountProducts(sorted.slice(0, 10));
    } catch (error) {
      console.error("❌ Error fetching discount products:", error);
    }
  };
  // Lấy sản phẩm mới nhất (trong vòng 1 tuần)
  const fetchNewProducts = async () => {
    try {
      ...
      // Lọc sản phẩm được tạo trong vòng 1 tuần
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      await Promise.all([fetchDiscountProducts(), fetchNewProducts()]);
      setLoading(false);
    };
    fetchData();
  }, []);

  const renderFlipCard = (product: Product) => {
        ...
        <div className="flip-card-inner">
          ...
          <div className="flip-card-front">
            <img
              className="title"
              src={
                product.images?.[0]?.url || product.variants?.[0]?.image?.url
              }
              alt={product.name}
            />
          </div>
          ...
          <div className="flip-card-back">
            <p className="discount">
              {discount > 0 ? `Giảm ${discount}%` : "Hết KM"}
            </p>

            <p className="title">{product.name}</p>

            {discount > 0 && <h4>{formatCurrency(price)}</h4>}

            <h3>{formatCurrency(finalPrice)}</h3>

            <div className="btn">
              <button onClick={() => navigate(`/detail-product/${product.id}`)}>
                Xem chi tiết
              </button>
              <button onClick={handleAddCart}>Mua hàng</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div className="homepage-container">
      <div className="slider" data-aos="fade-down">
        {sliderImages.map((image, index) => (
          <img
            ...
            alt={`Slide ${index + 1}`}
            className={
              index === currentIndex ? "slider-image active" : "slider-image"
            }
          />
        ))}
      </div>
      <h1 className="button-ship" data-aos="fade-up">
        <span className="button_lg">
          ...
          <span className="button_text">
            Giao Hàng Nhanh Chóng - Hoa Đẹp Tận Tay - Gọi Ngay AVICI Shop
          </span>
        </span>
      </h1>
      {pathChildren ? (
        ...
        <>
          <h2 className="message-2">HOA TƯƠI GIẢM ĐẾN 30%</h2>
            <div className="cart-product-container" data-aos="fade-up">
              {discountProducts.map(renderFlipCard)}
            </div>
          <h2 className="message-3">SẢN PHẨM MỚI</h2>
          <div className="cart-product-container" data-aos="zoom-in">
            {newProducts.map(renderFlipCard)}
          </div>
          ...
          <div className="content-brand">
            <div className="content-brand-left" data-aos="zoom-up-right">
              <div className="introduction">
                ...
                <p>
                  Shop Hoa Tươi Avici là một trong những tiệm hoa uy tín nhất
                  tại TP.Hà Nội, Việt Nam. Chúng tôi cung cấp dịch vụ đặt hoa
                  online giao tận nơi tại Hà Nội, trên các tỉnh thành phố khác.
                  Với hệ thống cửa hàng liên kết khắp các tỉnh trên toàn quốc,
                  chúng tôi có thể giúp bạn gửi tặng hoa cho người thân, bạn bè,
                  người yêu ở bất cứ đâu tại Việt Nam.
                </p>
              </div>
              <div className="trustworthy">
                <h3>Tại sao nên chọn Avici Shop?</h3>
                <p>
                  Không khó để bạn tìm được một cửa hàng giao online tận nơi.
                  Vậy tại sao nên chọn chúng tôi? Do chúng tôi marketing ư,
                  không phải lí do là:
                </p>
                <ul>
                  <li>Hoa đẹp vcl, màu thì hài hòa, cây nhà lá vườn</li>
                  <li>
                    Thiết kế chuẩn 30Shine, có khi phóng đại yêu cầu khách hàng
                  </li>
                  <li>Gửi hình ảnh hoa lá kèn + giá trước khi giao nha</li>
                  <li>
                    Đội ngũ florists với nhiều năm kinh nghiệm, tương đương số
                    lần tìm job intern
                  </li>
                </ul>
              </div>
            ...
            <div className="content-brand-right" data-aos="zoom-up-left">
              <div className="category">
                ...
                <p>
                  Đến với cửa hàng hoa Avici Shop, bạn có thể thoải mái lựa chọn
                  giữa hơn 500+ mẫu hoa tươi được thiết kế sẵn theo các chủ đề
                  như:{" "}
                </p>
                <ul>
                  <li>
                    <span>Hoa sinh nhật: </span>Hoa tặng sinh nhật vợ, bạn gái,
                    ba mẹ, anh chị, bạn bè, đối tác hay đồng nghiệp.
                  </li>
                  <li>
                    <span>Hoa cưới: </span>Hoa cầm tay cho cô dâu, hoa cài áo
                    chú rể, hoa trang trí giường cưới
                  </li>
                  <li>
                    <span>Hoa tốt nghiệp: </span>Hoa tặng bạn bè, người thân,
                    người yêu trong dịp tốt nghiệp
                  </li>
                  <li>
                    <span>Hoa tang lễ: </span>Hoa chia buồn gửi tới đám tang
                  </li>
                  <li>
                    <span>Hoa trang trí: </span>Hoa dùng để trên bàn, trang trí
                    decor phòng ngủ
                  </li>
                </ul>
                <p>
                  Ngoài ra, quý khách có thể yêu cầu hoa theo dịch vụ khác, hoặc
                  các mẫu hoa ở nước ngoài.
                </p>
              </div>
              <div className="guarantee">
                <h3>Cam kết với Khách Hàng</h3>
                <p>
                  Avici Shop hiểu rằng, hoa tươi dù không mang nhiều giá trị về
                  vật chất, nhưng mang lại ý nghĩa to lớn về mặt tinh thần. Mỗi
                  một bó hoa gửi đi rất nhiều tình cảm, thông điệp yêu thương mà
                  bạn muốn gửi đến những người thân. Chính vì thế, Avici Shop
                  luôn nỗ lực nâng cao chất lượng sản phẩm và dịch vụ để mang
                  lại cho bạn những trải nghiệm khó quên khi sử dụng. Shop cũng
                  xin cam kết:{" "}
                </p>
                <ul>
                ...
                <p>
                  Nếu bạn đang cần chúng tôi, chúng tôi sẵn sàng trực chờ 24/7,
                  gọi ngay 1900 càng đông càng sướng à nhầm 0333438120 để được
                  tư vấn hoặc đặt ngay hoa!!
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="footer-general">
        ...
        <div className="agency">
          <h3>Chi nhánh</h3>
          <p>
            <span>Cửa hàng chính: </span>phường Đại Kim, quận Hoàng Mai, TP.Hà
            Nội
          </p>
          <p>
            <span>Cửa hàng phụ: </span>đường Nhổn, quận Bắc Từ Liêm, TP.Hà Nội
          </p>
          <p>
            <span>Trang web online: </span>AviciFlower.vn
          </p>
